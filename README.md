...
## 前言
对于已经工作了的小伙伴，你把成是见过"责任链"这种面向对象的设计模式的，还在上学的小伙伴也不用着急，你迟早会接触到的。本文旨在让小白同学和
不太熟悉责任链的朋友能够迅速入门。
...
全都要你一个人打理，哪里一不注意出了乱子，脑袋就要搬家。2、行程多，所以增改起来太麻烦，一旦有改动圣上的行程表容易乱。毕竟行程写在一起，
条理不清。所以问题来啦，和大人您可怎么排圣上的行程呢？  
...
用类似递归的方法正序倒序各便历了一次（Filter或Interceptor的）数组。没有责任链模式的应用，你甚至都无法运行任何一个Java程序。因为类加载一般
遵循"双亲委派"机制的双亲委派机制，实际上是用类似递归的方法正序倒序各便历了一次Classloader类所构成的链表（这里可以参见一到算法题：Leetcode），
只不过其中的逻辑比较复杂，由于本文只是做一个责任链模式的简单入门，这里不做过多展开。  

综上，充分理解和应用责任链设计模式，对我们的日常工作和阅读源码都很有帮助，能让我们有效提高代码的扩展性和可读性。你也可以在